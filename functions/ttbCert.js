...
/**
 * The ttbonline.gov TLS endpoint serves ONLY its leaf certificate and omits the
 * "Entrust OV TLS Issuing RSA CA 2" intermediate. Browsers/curl recover via AIA
 * fetching, but Node's TLS stack does not, so it rejects the handshake with
 * UNABLE_TO_VERIFY_LEAF_SIGNATURE. We bundle the public intermediate here and add it
 * to the trusted CA set for the TTB request so the chain can be built to a root that
 * is already in Node's store. This is a public CA certificate (no secret).
 *
 * Source: http://crt.sectigo.com/EntrustOVTLSIssuingRSACA2.crt
 */
const TTB_INTERMEDIATE_PEM = `-----BEGIN CERTIFICATE-----
...
module.exports = { TTB_INTERMEDIATE_PEM };
