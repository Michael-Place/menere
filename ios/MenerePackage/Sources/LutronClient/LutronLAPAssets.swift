import Foundation

/// The well-known Lutron **Local Access Protocol (LAP)** pairing credentials (P15-C1 fix).
///
/// A Caséta / RA-family bridge only opens its LAP pairing session — and only pushes the physical
/// button-press status (`Body.Status.Permissions` containing `PhysicalAccess`) — to a client that
/// authenticates the port-8083 TLS socket with the bundled Lutron "Caséta Application" certificate.
/// This is a fixed, public, non-secret credential shipped verbatim by both canonical open
/// implementations:
///   • pylutron-caseta `src/pylutron_caseta/assets.py` (`LAP_CA_PEM` / `LAP_CERT_PEM` / `LAP_KEY_PEM`),
///   • lutron-leap-js `PairingClient.ts` (the bundled `assoc` cert/key/ca).
///
/// Our previous pairing socket connected **anonymously** (no client identity), so the bridge never
/// established the pairing session and never pushed the button status — the countdown always timed out
/// even though Michael pressed the button. Presenting this identity is the fix.
///
/// These are NOT secrets (they are identical for every Caséta bridge worldwide and published in the
/// above repos); they are the shared bootstrap credential the bridge trusts to *begin* pairing, after
/// which each client gets its own unique signed certificate.
enum LutronLAPAssets {

    /// The LAP CA that signs the bundled application cert (and, on Caséta, the bridge's server cert).
    static let caPEM = """
    -----BEGIN CERTIFICATE-----
    
    zq38av5E
    -----END CERTIFICATE-----
    """

    /// The bundled "Caséta Application" client certificate presented on the pairing socket.
    static let certPEM = """
    -----BEGIN CERTIFICATE-----
    
    B31ZKg0R+JaGyy6ZpE5UPLVyUtLlN93V2V8n66kR
    -----END CERTIFICATE-----
    """

    /// The bundled application cert's RSA private key (PKCS#1 `RSA PRIVATE KEY`).
    static let keyPEM = """
    -----BEGIN RSA PRIVATE KEY-----
    
    -----END RSA PRIVATE KEY-----
    """
}
